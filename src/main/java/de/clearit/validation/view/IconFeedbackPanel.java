 ...
 * <code>#repositionFeedBackComponents</code> after the child tree has been laid
...
public final class IconFeedbackPanel extends JLayeredPane {
   ...
   *           if model or content is {@code null}.
   */
   ...
   *
   * @throws NullPointerException
   *           if the feedback component or content component is {@code null}
   */
   ...
   * in the specified <code>keyMap</code>.
   * <p>
   *
    ...
    for (int i = 0; i < componentCount; i++) {
      ...
      if (isMarkable(child)) {
        if (isScrollPaneView(child)) {
          ...
          addFeedbackComponent(containerParent, (JComponent) child, keyMap, xOffset - containerParent.getX(), yOffset
   ...
   *         by the specified validation key map or
   *         <code>ValidationResult.EMPTY</code> if the component has no message
   *         key set, or <code>ValidationResult.EMPTY</code> if no result is
