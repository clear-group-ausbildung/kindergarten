...
public class PurchaseAnalysis extends AbstractView {
  ...
  protected JComponent buildPanel() {
    ...
    builder.addI15dROLabel("purchase.kindergartenProfit", CC.xy(9, 1));
    builder.add(kindergartenProfitField, CC.xy(11, 1));
    builder.addI15dROLabel("purchase.vendorPayout", CC.xy(13, 1));
    builder.add(vendorPayoutField, CC.xy(15, 1));

    return builder.getPanel();
  }

  private void initComponents() {
    itemCountField = KindergartenComponentFactory.createReadOnlyTextField(new IntegerToStringConverter(itemCountModel));
    itemSumField = KindergartenComponentFactory.createReadOnlyTextField(new DoubleToStringConverter(itemSumModel));
    kindergartenProfitField = KindergartenComponentFactory.createReadOnlyTextField(new DoubleToStringConverter(
        kindergartenProfitModel));
    vendorPayoutField = KindergartenComponentFactory.createReadOnlyTextField(new DoubleToStringConverter(
        vendorPayoutModel));
  }
