...
public final class VendorService extends AbstractResourceService<VendorBean, Vendor> {
  ...
  /**
   * Finds a VendorBean by the given {@code vendorNumber).
   * 
   * @param vendorNumber the vendor number to search for
   * 
   * @return the found VendorBean or {@code null}, if there was no search result
   */
  public VendorBean findByVendorNumber(Integer vendorNumber) {
    ...
    for (VendorBean oneVendor : allVendors) {
      ...
      for (VendorNumberBean oneVendorNumber : oneVendor.getVendorNumbers()) {
        // 2.1.1 -> entspricht die aktuelle VendorNumber dem uebergebenen vendorNumber
        // Parameter?
    ...
    LOGGER.exiting(VendorService.class.getName(), "findByVendorNumber(Integer vendorNumber)", new Object[] { result });
    return result;
  }

  /**
   * Creates a new vendor with the information of the given {@code bean}.
   * 
   * @param bean
   *          the vendor to create
   */
  public void createVendor(VendorBean bean) {
    createAsNew(bean);
  }
